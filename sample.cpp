...
int main()
{
    ...
    K4A_FAILED(k4a_device_open(0, &device));
    ...
    while (true)
    {
        ...
        int colorRows = k4a_image_get_height_pixels(colorImage);
        int colorCols = k4a_image_get_width_pixels(colorImage);

        cv::Mat color(colorRows, colorCols, CV_8UC4, (void*)colorBuffer, cv::Mat::AUTO_STEP);

        cv::imshow("random", color);
        cv::waitKey(0);

        uint8_t* depthBuffer = k4a_image_get_buffer(depthImage);
        int32_t depthRows = k4a_image_get_height_pixels(depthImage);
        int32_t depthCols = k4a_image_get_width_pixels(depthImage);

        cv::Mat depth(depthRows, depthCols, CV_16UC1, reinterpret_cast<uint16_t*>(depthBuffer));
        depth.convertTo(depth, CV_8UC3, 255.0 / 5000.0, 0.0); // 
        //cv::Mat backtorgb;
        //cv::cvtColor(depth, backtorgb, cv::COLOR_GRAY2BGR);
        ...
        for (int f = 0; f < hand.fingers().count(); f++)
        {
            ...
            //Get position x,y,z (Float)
            std::cout << transformedPosition.x << transformedPosition.y << transformedPosition.z << std::endl;
            //Get Direction x,y,z (Float)
            std::cout << transformedDirection.x << transformedDirection.y << transformedDirection.z << std::endl;
            //std::cout<< "Position: " <<transformedPosition << " Direction: " << transformedDirection << std::endl;
...
//    config.color_resolution = K4A_COLOR_RESOLUTION_720P;
//    config.depth_mode = K4A_DEPTH_MODE_NFOV_UNBINNED;
//    //config.depth_delay_off_color_usec = false;
//    k4a_calibration_t calibration;
//    k4a_device_get_calibration(device, config.depth_mode, config.color_resolution, &calibration);
//    transformation = k4a_transformation_create(&calibration);
//    k4a_device_start_cameras(device, &config);
...
//
//        // release capture
//        k4a_capture_release(capture); /* this */
//
//        // get mat
//        cv::Mat colorMat = k4a_get_mat(colorImage);
//        cv::imshow("color", colorMat);
//        cv::waitKey(0);
//        //cv::Mat depthMat = k4a_get_mat(depthImage);
//        //cv::imshow("color", depthMat);
//        //cv::waitKey(0);
//        // release image
//        k4a_image_release(colorImage);
//        //k4a_image_release(depthImage);
